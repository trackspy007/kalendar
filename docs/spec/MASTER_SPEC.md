...
**Synopsis-text sourcing priority specifically:** (1) Netflix's newsroom (`about.netflix.com/en/newsroom`) when it's a Netflix Original — real official English prose, but Netflix has no public API, so this is occasional manual/session reading, not an automated pull; (2) Wikipedia's CC BY-SA text, once an article exists; (3) otherwise, leave the synopsis field blank/TBA. DramaWiki's and MDL's own prose write-ups are never used as a synopsis source, not even paraphrased.
...
- No verbatim republishing of MDL-authored (or DramaWiki-authored) synopsis text or other prose content, ever — not verbatim, not paraphrased. This applies regardless of which API/page the data came through; structured facts (title, episode count, air dates, network, cast, genre) are fine to pull from either source, prose write-ups are not.
